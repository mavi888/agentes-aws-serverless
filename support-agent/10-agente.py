...
class LoggingPlugin(Plugin):
    ...
    def log_after_tool(self, event: AfterToolCallEvent) -> None:
        ...
        if event.exception is None:
            ...
            print(f"❌ Herramienta falló: {event.tool_use['name']} ({elapsed:.2f}s)")

# --- NUEVO: Plugin de reintentos ---
    ...
    def retry_on_failure(self, event: AfterToolCallEvent) -> None:
        ...
        if event.exception is not None:
            ...
            if retries < 3:
                ...
                print(f"⚠️ {tool_name} falló, reintentando ({retries + 1}/3)...")
                event.retry = True
            else:
                print(f"🔥 {tool_name} falló después de 3 intentos")
                self._retries[tool_name] = 0
        else:
            self._retries[tool_name] = 0


# --- Herramienta con fallback ---
knowledge_base = {
...
def buscar_solucion(problema: str) -> str:
    ...
    if random.random() < 0.8:
        print("💥 [SIMULACIÓN] Fallo de conexión a la base de conocimiento")
        raise ConnectionError("Timeout conectando a la base de conocimiento")
...
def main():
    ...
    # --- Agente completo con MCP y reintentos ---
    soporte = Agent(
