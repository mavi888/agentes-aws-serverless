...
# --- Función para generar receta estructurada ---
...
# --- NUEVO: Función para guardar receta en archivo ---
def guardar_receta_en_archivo(chef, receta: Recipe) -> None:
    """Muestra la receta y la guarda como JSON usando MCP filesystem."""
    print(f"  📖 {receta.nombre}")
    print(f"  ⏱️  {receta.tiempo_preparacion} | 🍽️ {receta.porciones} porciones | {receta.dificultad}")
    print(f"  🥗 Ingredientes: {', '.join(receta.ingredientes)}")
    print()

    filename = receta.nombre.lower().replace(" ", "_")
    for char in "áéíóú":
        filename = filename.replace(char, "aeiou"["áéíóú".index(char)])
    receta_json = receta.model_dump_json(indent=2)

    print("💾 Guardando receta en archivo...")
    try:
        chef(f"Usá la herramienta write_file para guardar este contenido en el path './recetas/{filename}.json':\n\n{receta_json}")
        print(f"  ✅ Receta guardada en ./recetas/{filename}.json")
    except Exception as e:
        print(f"  ⚠️ No se pudo guardar: {e}")

# --- NUEVO: MCP Filesystem para guardar/leer recetas ---
RECETAS_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "recetas")
os.makedirs(RECETAS_DIR, exist_ok=True)

filesystem_client = MCPClient(lambda: stdio_client(
    StdioServerParameters(
        command="npx",
        args=["-y", "@modelcontextprotocol/server-filesystem", RECETAS_DIR]
    )
))
...
def main():
    ...
    chef = Agent(
        ...
        1. mi_heladera — Para manejar los ingredientes disponibles del usuario
        2. current_time — Para saber la hora y sugerir recetas apropiadas al momento del día
        3. calculator — Para ajustar cantidades de ingredientes según la cantidad de personas
        4. Herramientas de filesystem (MCP) — Para guardar y leer recetas en archivos

        Tu forma de trabajar:
        1. Cuando el usuario menciona ingredientes, agregalos a la heladera usando mi_heladera
        2. Usá current_time para saber qué hora es y sugerir recetas apropiadas (desayuno, almuerzo, merienda, cena)
        3. Sugerí recetas basándote en los ingredientes disponibles
        4. Si te piden ajustar cantidades, usá calculator
        5. Si te preguntan por recetas guardadas, buscalas en el filesystem
        ...
        """,
        tools=[current_time, calculator, mi_heladera, filesystem_client],
    ...
    while True:
        ...
        if user_input.lower() in ["salir", "exit", "quit"]:
            ...
            # Generar receta estructurada si hubo conversación
            if chef.messages:
                print("\n📝 Generando receta estructurada...\n")
                receta = generar_receta(chef)
                if receta:
                    guardar_receta_en_archivo(chef, receta)
