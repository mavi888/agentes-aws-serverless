...
class KnowledgeBaseStack(Stack):
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        docs_bucket = s3.Bucket(self, "SupportAgentKBDocsBucket",
            bucket_name=f"soporte1-it-kb-docs-{self.account}",
            removal_policy=RemovalPolicy.DESTROY,
            auto_delete_objects=True,
        )

        # 2. Subir los documentos al bucket durante el deploy
        s3deploy.BucketDeployment(self, "SupportAgentDeployDocs",
            sources=[s3deploy.Source.asset("./kb-docs")],
            destination_bucket=docs_bucket,
        )

        # 3. Vector Bucket (S3 Vectors) — almacena los embeddings
        vector_bucket_name = f"soporte-it-vectors-{self.account}"
        vector_index_name = "soporte-it-index"

        vector_bucket = s3vectors.CfnVectorBucket(self, "SupportAgentVectorBucket",
            vector_bucket_name=vector_bucket_name,
        )

        # 4. Índice de vectores dentro del Vector Bucket
        #    Titan Embed Text v2 genera vectores de 1024 dimensiones
        vector_index = s3vectors.CfnIndex(self, "SupportAgentVectorIndex",
            vector_bucket_name=vector_bucket_name,
        ...
        kb_role = iam.Role(self, "SupportAgentKBRole",
            assumed_by=iam.ServicePrincipal("bedrock.amazonaws.com"),
            description="Rol para la Knowledge Base de soporte IT",
        )
        ...
        self.kb = bedrock.CfnKnowledgeBase(self, "SupportAgentSoporteKB",
            name="soporte-it-kb",
            description="Knowledge Base del agente de soporte IT",
        ...
        bedrock.CfnDataSource(self, "SupportAgentDataSource",
            knowledge_base_id=self.kb.attr_knowledge_base_id,
            name="soporte-it-docs",
        ...
        # Outputs
        CfnOutput(self, "SupportAgentKnowledgeBaseId",
            value=self.kb.attr_knowledge_base_id,
            export_name="SupportAgentKBId",
            description="ID de la Knowledge Base",
        )
        CfnOutput(self, "SupportAgentKnowledgeBaseArn",
            value=self.kb.attr_knowledge_base_arn,
            export_name="SupportAgentKBArn",
        )
        CfnOutput(self, "SupportAgentDocsBucketName",
